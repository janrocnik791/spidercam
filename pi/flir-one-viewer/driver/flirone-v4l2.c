...
// FLIR One *Pro* visible sensor is 1440x1080; the camera emits a JPEG of
// that size on EP 0x85. The v4l2loopback format MUST advertise the true
// resolution: if it under-declares (e.g. the gen-2 640x480), OpenCV/libjpeg
// size their MJPEG decode buffer too small for the real frame and abort with
// "premature end of data segment", yielding an all-black visual stream.
#define FRAME_WIDTH1  1440
#define FRAME_HEIGHT1 1080
...
// fb_proc2 is an 24bit RGB buffer
 ...
 {    
	...
	char EP81_error[50]="", EP83_error[50]="",EP85_error[50]=""; 
 	...
 	// NOTE: startv4l2() is intentionally NOT called here. The v4l2loopback
 	// output devices are opened once for the whole process lifetime in main().
 	// The FLIR One Pro periodically drops off USB (LIBUSB_ERROR_NO_DEVICE),
 	// which makes EPloop() return so main()'s while(1) can reconnect. If we
 	// re-opened the loopback devices on every reconnect, the second
 	// VIDIOC_S_FMT would fail with EBUSY (the previous producer handle, and
 	// the viewer's active capture, still hold the format), the assert would
 	// fire, and the whole driver would abort — freezing the web stream on its
 	// last frame. Keeping the loopback fds open across reconnects lets the
 	// stream resume live once the camera re-enumerates.
...
//        print_bulk_result("0x83",EP83_error, r, actual_length, buf); 
 ...
 out:
    //close the device
 	// devh is NULL when find_lvr_flirusb() couldn't open the camera (e.g. the
 	// Pro is mid-reset / powered down). Guarding here avoids a NULL-deref
 	// segfault so main()'s while(1) can simply retry until it comes back.
 	if (devh) {
...
int main(int argc, char **argv)
{
  ...
  // Open the v4l2loopback output devices once and keep them open. EPloop()
  // re-runs on every USB reconnect, but the loopback side stays stable so the
  // viewer's capture never breaks and the format is set exactly once.
  startv4l2();

  while (1)
  {
    EPloop(colormap);
    // EPloop only returns when the camera drops off USB. Back off briefly so
    // a persistently-absent camera doesn't spin this loop at 100% CPU.
    sleep(1);
  }
